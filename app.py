...
COURSE_OPTIONS = [
    # ── OPÇÃO 1: Babá — Realengo ─────────────────────────────────────────────
    {
        "id": "1",
        "local": "COZINHA COMUNITÁRIA DE REALENGO",
        "curso": "TREINAMENTO PARA BABÁ",
        "turma": "TURMA BABÁ 04/07",
        "dias_aula": "Quinta",
        "horario": "09h às 13h",
        "data_inicio": "04/07/2026",
        "encerramento": "04/07/2026",
        "endereco_curso": "📍Rua Capitão Teixeira, nº 583 - Realengo - Rio de Janeiro - CEP: 21755-000 - RJ",
    },
    # ── OPÇÃO 2: Baby Organizer — Estácio Ilha do Governador ─────────────────
    {
        "id": "2",
        "local": "FACULDADE ESTÁCIO DE SÁ - ILHA DO GOVERNADOR",
        "curso": "FORMAÇÃO DE ORGANIZAÇÃO BABY - BABY ORGANIZER",
        "turma": "TURMA BABY ORGANIZER 01/08",
        "dias_aula": "Sábado",
        "horario": "08h às 12h",
        "data_inicio": "01/08/2026",
        "encerramento": "01/08/2026",
        "endereco_curso": "📍Estrada do Galeão, nº 5.800 - Ilha do Governador - Rio de Janeiro - CEP: 21941-352 - RJ",
    },
    # ── OPÇÃO 3: Baby Organizer — Nova Sepetiba ───────────────────────────────
    {
        "id": "3",
        "local": "COMUNIDADE NOVA SEPETIBA",
        "curso": "FORMAÇÃO DE ORGANIZAÇÃO BABY - BABY ORGANIZER",
        "turma": "TURMA BABY ORGANIZER 07/08",
        "dias_aula": "Sexta",
        "horario": "09h às 13h",
        "data_inicio": "07/08/2026",
        "encerramento": "07/08/2026",
        "endereco_curso": "📍Avenida Dois, nº 360 - Nova Sepetiba - Rio de Janeiro - CEP: 23545-003 - RJ",
    },
]
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .wizard-fill { height: 100%; width: 25%; background: linear-gradient(90deg, #fc6a5b 0%, #ff8f84 100%); border-radius: 999px; transition: width 0.3s ease; }
        ...
        .cta-button, .submit-button { background: linear-gradient(90deg, #fc6a5b 0%, #ff8f84 100%); color: #fff; box-shadow: 0 10px 24px rgba(252,106,91,0.24); }
        ...
        .balao-erro { margin-top: 4px; padding: 10px 14px; border-radius: 14px; border: 1px solid #e55546; background: #fc6a5b; color: #fff; font-size: 0.92rem; font-weight: 700; line-height: 1.35; }
...
<body data-start-step="{{ current_step }}">
                ...
                <section class="wizard-panel" data-step="index">
                    <div class="hero-grid">
                        <div class="hero-card">
                            ...
                            <h1 class="hero-title">PROGRAMA PROFISSIONAL DO LAR 360º</h1>
                            <p class="hero-subtitle">Cuidando de quem cuida.</p>
                            <div class="hero-highlights">
                                <div class="hero-highlight" style="text-align:left;">
                                    <strong>CURSOS DISPONÍVEIS:</strong>
                                    📚 TREINAMENTO PARA BABÁ<br>
                                    📚 FORMAÇÃO DE ORGANIZAÇÃO BABY - BABY ORGANIZER
                                </div>
                                <div class="hero-highlight">
                                    <strong>BENEFÍCIOS</strong>
                                    ✅ 100% GRATUITO<br>
                                    📃 CERTIFICADO DE CONCLUSÃO<br>
                                    💼 CADASTRAMENTO ÀS VAGAS DE EMPREGO
                                </div>
                                <div class="hero-highlight">
                                    ...
                                    <span class="info-rotativo">
                                        ...
                                        <span class="info-item" style="display:none;">⚠️ Para Maiores de 18 Anos</span>
                                    </span>
                                    <script>
                                    document.addEventListener('DOMContentLoaded', function() {
                                        var items = document.querySelectorAll('.info-rotativo .info-item');
                                        var idx = 0;
                                        setInterval(function() {
                                            items.forEach(function(el, i) { el.style.display = (i === idx) ? 'inline' : 'none'; });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function validarCPF(cpf){var d=somenteDigitos(cpf);if(d.length!==11||/^(\d)\1+$/.test(d))return false;var soma=0,dig;for(var i=0;i<9;i++)soma+=Number(d.charAt(i))*(10-i);dig=(soma*10)%11;if(dig===10)dig=0;if(dig!==Number(d.charAt(9)))return false;soma=0;for(var i=0;i<10;i++)soma+=Number(d.charAt(i))*(11-i);dig=(soma*10)%11;if(dig===10)dig=0;return dig===Number(d.charAt(10));}
            function validarEmail(e){return/^[a-zA-Z0-9_.+-]+@((gmail|hotmail|outlook|yahoo)\.(com|com\.br))$/i.test((e||'').trim());}
            function idadePermitida(v){var p=(v||'').split('/');if(p.length!==3)return false;var dia=Number(p[0]),mes=Number(p[1])-1,ano=Number(p[2]),d=new Date(ano,mes,dia);if(isNaN(d.getTime())||d.getDate()!==dia||d.getMonth()!==mes||d.getFullYear()!==ano)return false;var hoje=new Date(),idade=hoje.getFullYear()-d.getFullYear();if(hoje.getMonth()-d.getMonth()<0||(hoje.getMonth()===d.getMonth()&&hoje.getDate()<d.getDate()))idade--;return idade>=18&&idade<=90;}
            ...
            function aplicarOpcaoCurso(optionId){
                var op=courseOptionsById[String(optionId)];
                if(!op){cursoInput.value='';turmaInput.value='';diasAulaInput.value='';horarioInput.value='';dataInicioInput.value='';encerramentoInput.value='';enderecoInput.value='';return;}
                localInput.value=String(op.id);cursoInput.value=op.curso;turmaInput.value=op.turma;diasAulaInput.value=op.dias_aula;horarioInput.value=op.horario;dataInicioInput.value=op.data_inicio;encerramentoInput.value=op.encerramento;enderecoInput.value=op.endereco_curso;setError('opcao_id','');
            }
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Wise:wght@400;700;900&display=swap" rel="stylesheet">
    <style>
        :root{--cor-principal:#fc6a5b;--cor-clara:#fff0ee;--cor-texto:#5b2f2b;--sombra-card:0 18px 55px rgba(252,106,91,0.18);}
        ...
        .main-header{border-bottom:4px solid #fc6a5b;background:rgba(255,255,255,0.92);}
        ...
        .wizard-fill{width:100%;height:100%;background:linear-gradient(90deg,#fc6a5b 0%,#ff8f84 100%);border-radius:999px;}
        ...
        .checkmark svg{width:100%;height:100%;stroke:#fc6a5b;fill:none;}
        .confirm-card h1{margin:0 0 10px;color:#fc6a5b;font-size:clamp(1.8rem,4vw,2.6rem);letter-spacing:-0.04em;}
        .protocol-box{margin:16px auto 12px;padding:14px;max-width:320px;border-radius:16px;background:#fff0ee;border:2px solid #fc6a5b;}
        .protocol-box strong{display:block;color:#fc6a5b;font-size:0.98rem;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.04em;}
        .protocol-box span{display:block;color:#fc6a5b;font-size:1.35rem;font-weight:900;letter-spacing:0.08em;word-break:break-all;}
        .next-steps{margin:16px auto 0;max-width:460px;padding:14px;border-radius:18px;background:#fff;border:1px solid #ffd8d2;}
        .next-steps h2{margin:0 0 12px;color:#fc6a5b;font-size:1.2rem;}
        ...
        .action-button.primary{background:linear-gradient(90deg,#fc6a5b 0%,#ff8f84 100%);color:#fff;box-shadow:0 10px 24px rgba(252,106,91,0.24);}
        .action-button.secondary{background:#fff;color:#fc6a5b;border:2px solid #fc6a5b;}
